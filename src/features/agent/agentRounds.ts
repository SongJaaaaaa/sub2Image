...
export function remapAgentRoundMentionsForPathChange(content: string, oldPath: AgentRound[], newPath: AgentRound[]) {
  ...
  const newIndexByRoundId = new Map(newPath.map((round, idx) => [round.id, idx + 1]))
...
export function deleteAgentRoundFromConversation(conversation: AgentConversation, roundId: string, now = Date.now()): AgentConversation {
  ...
  const rounds = conversation.rounds
    ...
    .map((round) => round.parentRoundId === roundId
      ? { ...round, parentRoundId: targetRound.parentRoundId ?? null }
      : round,
    )
  const messages = conversation.messages.filter((message) => message.roundId !== roundId)
...
export function getAgentRoundTaskIds(round: AgentRound, tasks: TaskRecord[]) {
  ...
  return uniqueIds([
    ...round.outputTaskIds,
    ...tasks.filter((task) => task.agentRoundId === round.id).map((task) => task.id),
