...
- 用户导入单个 `.md` Skill、本地持久化和删除。
- 已登录用户可手动保存或移出云端；开启自动保存后，新导入的 Skill 会自动保存。
- 登录后恢复云端 Skill；同 ID 内容冲突时由用户选择本地或云端版本，不静默覆盖。
...
- 不实现 zip、目录、脚本资源、在线市场或多 Skill 组合。

## 快速定位

- `types.ts`：Agent Skill 契约。
- `registry.ts`：Markdown 解析、自动注册、用户文件校验和本地持久化。
- `../features/cloud/runtime.ts`：用户 Skill 的云端保存、恢复和冲突处理。
