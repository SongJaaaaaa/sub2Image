...
import type { AgentInputDraft, AppState } from './types'
...
export function orderImagesWithMaskFirst(images: InputImage[], maskTargetImageId: string | null | undefined) {
  ...
  const idx = images.findIndex((img) => img.id === maskTargetImageId)
...
export function normalizeAgentInputDrafts(value: unknown, conversations: AgentConversation[]): Record<string, AgentInputDraft> {
  ...
  for (const [id, draft] of Object.entries(value)) {
    ...
    if (!isEmptyAgentInputDraft(normalized)) drafts[id] = normalized
  }
  return drafts
}
...
export function cleanStaleAgentInputDrafts(drafts: Record<string, AgentInputDraft>, activeConversationId: string | null, now = Date.now()) {
  ...
  for (const [id, draft] of Object.entries(drafts)) {
    if (id === activeConversationId || (draft.updatedAt ?? now) >= cutoff) next[id] = draft
  }
  return next
}
...
export function saveActiveAgentInputDrafts(state: Pick<AppState, 'appMode' | 'activeAgentConversationId' | 'agentInputDrafts' | 'prompt' | 'inputImages' | 'maskDraft' | 'maskEditorImageId'>) {
...
export function saveGalleryInputDraft(state: Pick<AppState, 'appMode' | 'galleryInputDraft' | 'prompt' | 'inputImages' | 'maskDraft' | 'maskEditorImageId'>) {
...
export function getPersistableAgentInputDrafts(state: AppState) {
  ...
  const ids = new Set(state.agentConversations.map((conversation) => conversation.id))
  const persistable: Record<string, AgentInputDraft> = {}
  for (const [id, draft] of Object.entries(drafts)) {
    if (!ids.has(id) || isEmptyAgentInputDraft(draft)) continue
    persistable[id] = {
